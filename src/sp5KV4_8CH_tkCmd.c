...
static void cmdHelpFunction(void)
{
	...
	FreeRTOS_write( &pdUART1, cmd_printfBuff, sizeof(cmd_printfBuff) );
	snprintf_P( cmd_printfBuff,sizeof(cmd_printfBuff),PSTR("  (SM) clearQ{0..3}\r\n\0"));
	FreeRTOS_write( &pdUART1, cmd_printfBuff, sizeof(cmd_printfBuff) );
	...
	FreeRTOS_write( &pdUART1, cmd_printfBuff, sizeof(cmd_printfBuff) );
	snprintf_P( cmd_printfBuff,sizeof(cmd_printfBuff),PSTR("  dcd, rtc \r\n\0"));
	FreeRTOS_write( &pdUART1, cmd_printfBuff, sizeof(cmd_printfBuff) );
	...
	FreeRTOS_write( &pdUART1, cmd_printfBuff, sizeof(cmd_printfBuff) );
	snprintf_P( cmd_printfBuff,sizeof(cmd_printfBuff),PSTR("  (SM) adc {0..7}\r\n\0"));
	FreeRTOS_write( &pdUART1, cmd_printfBuff, sizeof(cmd_printfBuff) );
...
static void cmdRedialFunction(void)
{
	...
	while ( xTaskNotify(xHandle_tkGprsTx ,TKG_PARAM_RELOAD , eSetBits ) != pdPASS ) {
		vTaskDelay( ( TickType_t)( 100 / portTICK_RATE_MS ) );
	}
...
StatBuffer_t pxFFStatBuffer;
	...
	for ( channel = 0; channel < NRO_DIGITAL_CHANNELS; channel++) {
		snprintf_P( cmd_printfBuff,sizeof(cmd_printfBuff),PSTR("  d%02d{%.02f p/p} (%s)\r\n\0"), channel, systemVars.magPP[channel],systemVars.dChName[channel]);
		FreeRTOS_write( &pdUART1, cmd_printfBuff, sizeof(cmd_printfBuff) );
	}
	/* VALUES --------------------------------------------------------------------------------------- */
	...
	// TimeStamp.
	pos += snprintf_P( &cmd_printfBuff[pos], sizeof(cmd_printfBuff),PSTR( "TimeStamp: %04d%02d%02d,"),Cframe.rtc.year,Cframe.rtc.month,Cframe.rtc.day );
...
char *p;
	...
	// ADC
	// read adc channel
	if (!strcmp_P( strupr(argv[1]), PSTR("ADC\0")) && ( systemVars.wrkMode == WK_SERVICE) ) {
		pv_cmdRdADC();
		return;
	}
...
u08 argc;
	...
	if (!strcmp_P( strupr(argv[1]), PSTR("CLEARQ\0")) && ( systemVars.wrkMode == WK_SERVICE) ) {
		switch ( atoi(argv[2])  ) {
		case 0:
			cbi( D0_CLR_PORT, D0_CLR_PIN );
			vTaskDelay( ( TickType_t)( 1) );
			sbi( D0_CLR_PORT, D0_CLR_PIN );
			break;
		case 1:
			cbi( D1_CLR_PORT, D1_CLR_PIN );
			vTaskDelay( ( TickType_t)( 1) );
			sbi( D1_CLR_PORT, D1_CLR_PIN );
			break;
		case 2:
			cbi( D2_CLR_PORT, D2_CLR_PIN );
			vTaskDelay( ( TickType_t)( 1) );
			sbi( D2_CLR_PORT, D2_CLR_PIN );
			break;
		case 3:
			cbi( D3_CLR_PORT, D3_CLR_PIN );
			vTaskDelay( ( TickType_t)( 1) );
			sbi( D3_CLR_PORT, D3_CLR_PIN );
			break;
		}
		return;
	}
...
u08 pin;
	...
	switch( atoi(argv[2] )) {
	case 0:
		pin = D0_IN_PIN & _BV(D0_IN);
		break;
	case 1:
		pin = D1_IN_PIN & _BV(D1_IN);
		break;
	case 2:
		pin = D2_IN_PIN & _BV(D2_IN);
		break;
	case 3:
		pin = D3_IN_PIN & _BV(D3_IN);
		break;
	default:
		snprintf_P( cmd_printfBuff,sizeof(cmd_printfBuff),PSTR("ERROR\r\n\0"));
		FreeRTOS_write( &pdUART1, cmd_printfBuff, sizeof(cmd_printfBuff) );
...
u08 pos, channel;
	...
	while(1) {
		...
		// Digital
		for ( channel = 0; channel < NRO_DIGITAL_CHANNELS; channel++ ) {
			pos += snprintf_P( &cmd_printfBuff[pos], ( sizeof(cmd_printfBuff) - pos ), PSTR("%sP=%d"), systemVars.dChName[channel],Aframe.dIn.pulses[channel] );
